(function (global) {
  ...
  function photoRow(item) {
    var slots = ['<div class="ml-slot is-filled" data-listing-photo><img src="' + escapeHtml(item.thumb) + '" alt="' + escapeHtml(item.name) + 'の出品写真"><span class="ml-slot-num">1</span></div>', '<div class="ml-slot" data-listing-photo><span class="ml-slot-cam" aria-hidden="true">◎</span><span class="ml-slot-num">2</span></div>'];
    var index;
    for (index = 3; index <= 5; index += 1) {
      slots.push('<div class="ml-slot" data-listing-photo><span class="ml-slot-num">' + index + '</span></div>');
    }
    return '<div class="ml-photos" aria-label="商品の写真">' + slots.join('') + '</div>';
  }

  function navRow(label, value, pill, note) {
    return ['<div class="ml-row" data-listing-nav>', pill ? '<span class="ml-pill">' + escapeHtml(pill) + '</span>' : '', '<div class="ml-row-main">', '<span class="ml-row-label">' + escapeHtml(label) + '</span>', '<span class="ml-row-value">' + escapeHtml(value) + '</span>', '<span class="ml-chevron" aria-hidden="true">›</span>', '</div>', note ? '<span class="ml-row-note">' + escapeHtml(note) + '</span>' : '', '</div>'].join('');
  }

  function saleTypeBlock(item) {
    var fee = Math.round(item.marketPrice * 0.1);
    var profit = item.marketPrice - fee;
    return ['<div class="ml-block">', '<h2 class="ml-heading">販売タイプ</h2>', '<div class="ml-option" data-listing-sale-type>', '<span class="ml-radio" aria-hidden="true"></span>', '<div class="ml-option-copy"><strong>オークション形式</strong><small>思わぬ価格で売れるかも！</small></div>', '</div>', '<div class="ml-option is-selected" data-listing-sale-type>', '<div class="ml-option-head">', '<span class="ml-radio is-on" aria-hidden="true"></span>', '<div class="ml-option-copy"><strong>価格を設定する</strong></div>', '</div>', '<div class="ml-price-rows">', '<div class="ml-price-row"><span>販売価格</span><strong>' + AppState.formatYen(item.marketPrice) + '</strong></div>', '<div class="ml-price-row"><span>販売手数料</span><b>−¥' + yen(fee) + '</b></div>', '<div class="ml-price-row is-last"><span>販売利益</span><b>¥' + yen(profit) + '</b></div>', '</div>', '</div>', '</div>'].join('');
  }
  ...
  function listingDone() {
    return ['<section class="screen listing-screen ml-done-screen">', '<div class="ml-done-card" role="dialog" aria-modal="true" aria-labelledby="ml-done-title">', '<div class="ml-done-illust" aria-hidden="true">💐</div>', '<h2 id="ml-done-title">出品が完了しました</h2>', '<p class="ml-done-note">売れたら売上金は<strong>推しポート</strong>に反映されます</p>', '<button type="button" class="ml-done-primary" data-finish-listing>出品した商品をみる</button>', '<button type="button" class="ml-done-secondary" data-listing-again>続けて出品する</button>', '<button type="button" class="ml-done-text" data-listing-share>商品をシェアする</button>', '</div>', '</section>'].join('');
  }
  ...
  global.Screens.listing = {
    ...
    render: function () {
      ...
      if (analysis && state.postedItemId === item.id) {
        ...
        displayItem = Object.assign({}, item, {
          thumb: state.post.imageUrl || AI_RESULTS['acsta2.png'].image,
    ...
    bind: function (root) {
      ...
      if (template) {
        template.addEventListener('click', function () {
          AppState.showToast('テンプレート入力はデモでは省略しています');
        });
      }
