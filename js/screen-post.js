(function (global) {
  ...
  function renderSelect() {
    return ['<section class="screen screen-post">', postHeader('1 / 2'), '<div class="screen-scroll post-select-wrap">', '<div class="post-lead"><span class="big-sparkle">✦</span><h2>新しい「好き」を<br>記録しよう</h2><p>写真からグッズ名と相場をAIが提案します</p></div>', '<label class="upload-dropzone" for="post-file">', '<img src="assets/img/acsta2.png" alt="デモ用のステラ アクリルスタンド">', '<span class="upload-overlay"><b>カメラロールから選ぶ</b><small>選んだ写真をAIが分析します</small></span>', '</label>', '<input class="sr-only" id="post-file" data-post-file type="file" accept="image/*">', '<div class="privacy-note"><span>✧</span><p><strong>写真は公開前に確認できます</strong><br>AI解析結果は自由に直せます</p></div>', '</div>', '</section>'].join('');
  }

  function renderAnalyzing(state) {
    var photo = state.post.imageUrl || 'assets/img/acsta2.png';
    return ['<section class="screen screen-post">', postHeader('AI解析'), '<div class="analysis-stage">', '<div class="analysis-photo"><img src="' + escapeHtml(photo) + '" alt="解析中の商品写真"><span class="scan-line"></span><i class="corner tl"></i><i class="corner tr"></i><i class="corner bl"></i><i class="corner br"></i></div>', '<div class="ai-spinner" aria-hidden="true"><span></span></div>', '<h2>AIが商品を分析中…</h2>', '<p>写真から出品情報を作成しています</p>', '<div class="thinking-dots" aria-hidden="true"><i></i><i></i><i></i></div>', '</div>', '</section>'].join('');
  }

  function renderResult(state) {
    var fallback = AI_RESULTS['acsta2.png'];
    ...
    var tags = analysis
      ? [analysis.category, analysis.brand, analysis.color].filter(function (tag) {
          return Boolean(tag);
        })
      : fallback.tags;
    var headNote = analysis ? 'Gemini · 確信度 ' + Math.round(analysis.confidence * 100) + '%' : '仕込みデータ';
      ...
      missing.length
        ? [
            ...
            '<ul>' +
              missing
                .map(function (key) {
                  return '<li>' + escapeHtml(missingFieldLabel(key)) + 'を確認してください</li>';
                })
                .join('') +
              '</ul>',
            '</div>'
  ...
  function renderComplete(state) {
    ...
    var thumb = item ? item.thumb : 'assets/img/item-acsta.png';
    var label = item ? item.shortName || item.name : 'ステラのアクスタ';

    var card = isDuplicate
      ? ['<article class="duplicate-alert">', '<div class="alert-kicker"><span>⚡</span> 重複アイテムを検出</div>', '<div class="alert-body"><img src="' + escapeHtml(thumb) + '" alt="' + escapeHtml(label) + '"><div><h3>' + escapeHtml(label) + '、<em>' + item.count + '個目</em>を検出。</h3><p>次のオタクに<br>継承しませんか?</p></div></div>', '<button type="button" class="inherit-button" data-open-draft>継承ドラフトを見る <span>›</span></button>', '<small>メルカリのあんしん取引へ移動します</small>', '</article>'].join('')
      : ['<article class="duplicate-alert">', '<div class="alert-kicker"><span>✦</span> 資産に追加しました</div>', '<div class="alert-body"><img src="' + escapeHtml(thumb) + '" alt="' + escapeHtml(label) + '"><div><h3>' + escapeHtml(label) + '</h3><p>手放すなら<br>いま継承に出せます</p></div></div>', '<button type="button" class="inherit-button" data-open-draft>継承ドラフトを見る <span>›</span></button>', '<small>メルカリのあんしん取引へ移動します</small>', '</article>'].join('');

    return ['<section class="screen screen-post post-complete-screen">', postHeader('完了'), '<div class="screen-scroll complete-scroll">', '<div class="success-block"><div class="success-orb">✓<span>✦</span></div><h2>投稿しました。</h2><p>ポートフォリオに追加 <strong>+' + state.post.count + '点</strong></p></div>', card, '<button type="button" class="text-button" data-post-again>もう1件投稿する</button>', '</div>', '</section>'].join('');
  }

  global.Screens.post = {
    key: function (state) {
      return state.post.stage;
    },
    ...
    bind: function (root) {
      ...
      if (caption) {
        caption.addEventListener('input', function () {
          AppState.setPostCaption(caption.value);
        });
      }
