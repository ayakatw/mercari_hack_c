(function (global) {
    ...
    function renderSelect() {
        return [
            ...
            '<img src="assets/img/acsta2.png" alt="デモ用のステラ アクリルスタンド">',
    ...
    function renderAnalyzing(state) {
        ...
        return [
            ...
            '<div class="analysis-photo"><img src="' +
                escapeHtml(photo) +
                '" alt="解析中の商品写真"><span class="scan-line"></span><i class="corner tl"></i><i class="corner tr"></i><i class="corner bl"></i><i class="corner br"></i></div>',
    ...
    function renderResult(state) {
        ...
        var tags = analysis
            ? [analysis.category, analysis.brand, analysis.color].filter(
                  function (tag) {
                      return Boolean(tag);
                  },
              )
        ...
        var matched =
            analysis && analysis.matchedItemId
                ? AppState.getItem(analysis.matchedItemId)
                : null;
        ...
        return [
            ...
            state.post.analysisError
                ? '<div class="ai-error-note"><span>!</span><p>' +
                  escapeHtml(state.post.analysisError) +
                  '</p></div>'
                : '',
            '<div class="ai-result-card">',
            '<div class="ai-result-head"><span>✦ AI解析結果</span><small>' +
                escapeHtml(headNote) +
                '</small></div>',
            ...
            '<div><h2>' +
                ...
                '</strong></p></div>',
            '</div>',
            '<div class="market-box"><span>現在の相場</span><strong>' +
                AppState.formatYen(price) +
                '</strong><small>' +
                      ...
                      '<ul>' +
                          missing
                              .map(function (key) {
                                  return (
                                      '<li>' +
                                      escapeHtml(missingFieldLabel(key)) +
                                      'を確認してください</li>'
                                  );
                              })
                              .join('') +
            ...
            '<label class="caption-field">キャプション<textarea data-caption rows="3" placeholder="お迎えした気持ちを書こう">' +
                escapeHtml(state.post.caption) +
                '</textarea></label>',
            '</div>',
    ...
    function renderComplete(state) {
        ...
        return [
            ...
            '<div class="success-block"><div class="success-orb">✓<span>✦</span></div><h2>投稿しました。</h2><p>ポートフォリオに追加 <strong>+' +
                AppState.formatYen(addedValue) +
                '</strong></p></div>',
            card,
        ...
        bind: function (root) {
            ...
            if (draft) {
                draft.addEventListener('click', function () {
                    AppState.prepareListing(
                        AppState.getState().postedItemId || 'stella-acsta',
                    );
                });
            }
