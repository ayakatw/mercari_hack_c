(function (global) {
  ...
  function postCard(post) {
    ...
    var avatarMarkup = user.tappable ? '<button class="avatar-button is-tappable" type="button" data-profile="' + user.handle + '" aria-label="' + user.name + 'さんのプロフィールを見る"><img src="' + post.avatar + '" alt=""></button>' : '<span class="avatar-button" aria-hidden="true"><img src="' + post.avatar + '" alt=""></span>';
    ...
    return [
      ...
      post.giveaway ? '<button class="want-button' + (state.requestSent ? ' is-sent' : '') + '" type="button" data-request aria-label="このグッズを欲しいと伝える">' + (state.requestSent ? '送信済み ✓' : '欲しい') + '</button>' : '',
      '</div>',
      '<p class="feed-caption"><strong>' + user.name + '</strong> ' + post.caption + '</p>',
      '<div class="product-tags">' +
        post.tags
          .map(function (tag) {
            return '<span role="button" tabindex="0" data-explore-tag="' + tag + '" aria-label="' + tag + 'の投稿を探す"># ' + tag + '</span>';
          })
          .join('') +
        '</div>',
  ...
  global.Screens.home = {
    render: function () {
      return ['<section class="screen screen-home">', '<header class="top-app-bar">', '<div><span class="eyebrow">MERCARI presents</span><h1 class="wordmark">推しポート<span>✦</span></h1></div>', '<button type="button" class="round-icon-button settings-trigger" data-open-settings aria-label="表示のカスタマイズを開く"><span aria-hidden="true">◐</span><i class="settings-trigger-dot" aria-hidden="true"></i></button>', '</header>', '<div class="screen-scroll feed-list">', '<section class="feed-intro">', '<span class="mini-pill">STELLIGHT</span>', '<h2>今日も推しが尊い。<span>✦</span></h2>', '<p>好きの記録を、未来の出会いへ。</p>', '</section>', POSTS.map(postCard).join(''), '<p class="end-message">ここまで見ました ✦</p>', '</div>', '</section>'].join('');
    },
    ...
    render: function () {
      ...
      return [
        ...
        grid
          .slice(0, 9)
          .map(function (post) {
            return '<img src="' + post.image + '" alt="' + post.tags[0] + 'の投稿" loading="lazy" role="button" tabindex="0" data-profile-post aria-label="' + post.tags[0] + 'の投稿を見る">';
          })
          .join(''),
        '</div>',
        '</div>',
