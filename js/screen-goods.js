(function (global) {
  ...
  function goodsRow(item, showPrices) {
    ...
    return ['<article class="asset-row">', '<div class="asset-thumb-wrap"><img src="' + escapeHtml(item.thumb) + '" alt=""><span class="count-chip">×' + item.count + '</span></div>', '<div class="asset-copy">', '<div class="asset-name-line"><h3>' + escapeHtml(item.name) + '</h3>' + (item.duplicate ? '<span class="duplicate-badge">×2</span>' : '') + (item.status === 'listed' ? '<span class="listed-badge">出品中</span>' : '') + '</div>', withPrice ? '<p>継承時の相場 <strong>' + AppState.formatYen(item.marketPrice) + '</strong></p>' : '', '</div>', '<div class="asset-actions">', withPrice ? '<span class="trend ' + (rising ? 'up' : 'down') + '">' + (rising ? '+' : '') + trendLabel(item) + '%</span>' : '', '<button type="button" class="goods-inherit-button" data-inherit="' + escapeHtml(item.id) + '">継承する</button>', '</div>', '</article>'].join('');
  }

  global.Screens.goods = {
    render: function () {
      var state = AppState.getState();
      var duplicateCount = state.items.filter(function (item) {
        return item.duplicate;
      }).length;
      var listedCount = state.items.filter(function (item) {
      ...
      return [
        ...
        listedCount ? ['<div class="goods-price-row">', '<div><label>相場を表示</label><small>継承中の' + listedCount + '点に表示します</small></div>', '<button type="button" class="switch' + (state.showPrices ? ' is-on' : '') + '" data-toggle-prices aria-pressed="' + state.showPrices + '" aria-label="相場の表示を切り替える"><span></span></button>', '</div>'].join('') : '',
        '<div class="asset-section-head"><div><h2>グッズ一覧</h2><p>重複したグッズは次のオタクへ継承できます</p></div><span>' + state.items.length + '点</span></div>',
        '<div class="asset-list">' +
          state.items
            .map(function (item) {
              return goodsRow(item, state.showPrices);
            })
            .join('') +
          '</div>',
