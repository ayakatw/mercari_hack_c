(function (global) {
  ...
  function reviewRow(result, count) {
    ...
    return [
      '<div class="recognition-row">',
        ...
        '<div class="mini-stepper"><button type="button" data-tutorial-count="' + result.itemId + '" data-delta="-1" aria-label="' + result.name + 'を減らす">−</button><strong>' + count + '</strong><button type="button" data-tutorial-count="' + result.itemId + '" data-delta="1" aria-label="' + result.name + 'を増やす">＋</button></div>',
      '</div>'
    ].join('');
  }
  ...
  function renderReview(state) {
    ...
    return [
      '<section class="tutorial-screen tutorial-review">',
        ...
        backButton('capture'),
        '<div class="review-hint" role="button" tabindex="0" data-apply-estimated-theme aria-label="AI推定の推しカラー「' + estimated.name + '」を適用"><span>🎨</span><p>祭壇の色味から、あなたの推しカラーは<strong>' + estimated.hue + '「' + estimated.name + '」</strong>と推定。<br>タップで今すぐ着せ替えできます</p></div>',
  ...
  global.Screens.tutorial = {
    key: function (state) { return visibleStage(state); },
    ...
    render: function () {
      ...
      if (stage === 'value') { return renderValue(); }
    ...
    bind: function (root) {
      ...
      if (estimatedTheme) {
        var applyEstimatedTheme = function () { setThemeWithFeedback(Theme.DEFAULT_THEME); };
