(function (global) {
  ...
  function shrineModal(total) {
    ...
    return ['<div class="modal-layer shrine-layer" data-close-shrine>', '<section class="modal-sheet shrine-sheet" role="dialog" aria-modal="true" aria-labelledby="shrine-title">', '<div class="sheet-handle" data-close-shrine-handle aria-hidden="true"></div>', '<button type="button" class="modal-close" data-close-shrine-button aria-label="閉じる">×</button>', '<div class="share-preview" data-shrine-preview>', '<div class="share-brand">推しポート <span>✦</span></div>', '<img src="assets/img/saidan.png" alt="りなの祭壇">', '<div class="share-copy"><span>MY STELLIGHT ALTAR</span><h2 id="shrine-title">My祭壇</h2><strong>' + total + '点</strong><p>好きの記録が、わたしのポートフォリオ。</p></div>', '<div class="share-sparkles" aria-hidden="true">✦　⋆　✧</div>', '</div>', '<p class="share-help">祭壇カードをシェアできます</p>', '<button type="button" class="x-share-button" data-share-x><span>𝕏</span> Xでシェア</button>', '</section>', '</div>'].join('');
  }

  global.Screens.mypage = {
    render: function () {
      var state = AppState.getState();
      var baseImages = ['assets/img/saidan.png', 'assets/img/item-badge.png', 'assets/img/item-plush.png', 'assets/img/item-penlight.png', 'assets/img/item-acsta.png', 'assets/img/item-album.png', 'assets/img/item-card.png', 'assets/img/item-hoodie.png', 'assets/img/tl-post.png'];
      ...
      return [
        ...
        grid
          .map(function (image, index) {
            return '<img src="' + image + '" alt="りなの投稿 ' + (index + 1) + '" loading="lazy" data-grid-post="' + (index + 1) + '">';
          })
          .join(''),
        '</div>',
        '</div>',
        state.shrineCardOpen
          ? shrineModal(
    ...
    bind: function (root) {
      ...
      if (avatar) {
        avatar.addEventListener('click', function () {
          AppState.showToast('プロフィール画像の変更はデモでは省略しています');
        });
      }
      var postStat = root.querySelector('[data-post-stat]');
      if (postStat) {
        postStat.addEventListener('click', function () {
          AppState.showToast('投稿グリッドを表示しています（デモ）');
        });
      }
      ...
      if (preview) {
        preview.addEventListener('click', function () {
          AppState.showToast('祭壇カードのシェア画像プレビューです（デモ）');
        });
      }
